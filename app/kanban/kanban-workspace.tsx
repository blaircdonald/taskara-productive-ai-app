...
type WorkspaceProps = { boards: Board[]; selectedBoardId: number | null; columns: Column[]; tasks: Task[]; labels: Label[]; collaborators: Collaborator[] };
...
function KanbanWorkspaceContent({ boards, selectedBoardId, columns: initialColumns, tasks: initialTasks, labels: initialLabels, collaborators }: WorkspaceProps) {
  const router = useRouter();
...
function TaskDialog({ task, labels, ownerExtras, pending, onClose, onSave, onDelete, onCreateLabel }: { task?: Task; labels: Label[]; ownerExtras: boolean; pending: boolean; onClose: () => void; onSave: (input: TaskDraft) => void; onDelete?: () => void; onCreateLabel: (input: { name: string; color: string }) => Promise<Label> }) {
