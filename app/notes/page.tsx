...
export default async function NotesPage({ searchParams }: { searchParams: Promise<{ note?: string; trash?: string }> }) {
  ...
  const summaries = await db.select({
    id: notes.id, title: notes.title, color: notes.color, categoryId: notes.categoryId, isPinned: notes.isPinned,
  ...
  return <AppShell><NotesWorkspace notes={summaries.map(serialize)} trash={trash.map(serialize)} selected={selected ? serialize(selected) : null} trashOpen={params.trash === "1"} categories={categories} autoSave={settings.noteAutoSave} aiRefineEnabled={settings.aiProcessingEnabled && settings.aiRefineEnabled} /></AppShell>;
}
