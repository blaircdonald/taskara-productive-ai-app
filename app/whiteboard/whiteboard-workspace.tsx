...
export function WhiteboardWorkspace({ boards, selected, aiDiagramEnabled }: { boards: Summary[]; selected: Board | null; aiDiagramEnabled: boolean }) {
    ...
    <div className="mx-auto flex h-full w-full max-w-[1900px] min-w-0 overflow-hidden rounded-2xl border border-stone-200/80 bg-white shadow-[0_20px_60px_rgba(120,90,60,0.12)]">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col bg-[#fffdfa]">
        <header className="z-20 flex min-h-16 shrink-0 items-center gap-2 border-b border-stone-200/80 bg-white/95 px-3 py-2 backdrop-blur sm:px-4">
          ...
          {aiDiagramEnabled && <button onClick={() => setAiOpen(true)} className="flex h-9 items-center gap-2 rounded-xl bg-amber-50 px-2.5 text-xs font-semibold text-amber-800 hover:bg-amber-100 sm:px-3"><Sparkles className="h-4 w-4" /><span className="hidden sm:inline">AI Diagram</span></button>}
